...
## 轮 91+:产品体验可见性 + 员工即真人 + 记忆驱动 worker(2026-06-11)

> 用户打断,转向产品体验:"看不到 AI 在干什么、人物入口太弱、没沉浸感、信息没层级"。主线:让
> AI 在画面上**活**起来 + 可见,同时核心能力继续往"聪明协作"深处走。

- **经理思考可见(根因修)** — ClaudeBrain.decide() 把 claude 的 OutputChunk 收集后**丢弃**,
  经理思考全程黑箱(用户"不知道在不在用 AI"的根因)。修:加思考流回调 emit manager-thinking,
  点经理 → Worksurf 实时看思路。real 验证:真 claude 经理思考流是**自然语言推理**(不是 fake JSON)。
- **经理思考动画** — 头顶冒思考点(复用 thinkpulse),不点开扫一眼也知道 AI 在决策。
- **AI 输出美化 + 全去 emoji** — MarkdownLite(代码块/行内码/列表)渲染 worker/经理思考;经理思考
  剥决策 JSON 只留自然语言;全 UI 清 emoji(用户强约束)。
- **任务链路条 + 建筑可交互** — TaskJourney(入队→执行→验证→交付,当前脉动);点办公室建筑开对应
  面板 + 预算楼实时今夜花费 → 减少右上角依赖;菜单入口按职责分组。
- **视觉修复** — 人物名字移屏幕空间浮层(2K/DPR 不糊)、审计位置、树静止。经理坐姿:盲调 6 轮,
  bridge headless 缩放失效、像素级 sprite 看不清 → 止损回干净站姿(见 [[bridge-shot-visual-debug]]:
  叠层面板/sprite 细节 bridge 看不清,以 DOM 验证为准、别盲调)。
- **★ 员工即真人(聪明协作闭环)** — 员工专长 + CEO 给的工作准则注入 worker 干活(配置生效);
  项目记忆注入 worker(约定/教训,不重蹈覆辙);战绩可见(身份卡:干过N/成功率/花费);**战绩派活**
  (专长命中后在对口的人里按战绩档挑、同档负载均衡、新人给机会)。配置+记忆+战绩+调度闭环。
- **★ real 验证记忆驱动 worker** — 插约定"新函数必须加 q_ 前缀",任务**只说**"加斐波那契函数"、
  没提前缀 → 真 claude worker 产出 `def q_fibonacci(...)`。记忆驱动 worker **真实生效**
  (simulate fake worker 不读 prompt,验证不了)。$0.178。

### 记忆驱动从"记忆→经理决策"扩到"记忆→worker 执行";聪明协作做成"对的人+靠谱的人干活"。

## 轮 100+:记忆/可信度/战绩接进决策 + real 验证经理基于记忆改写任务(2026-06-11)

- **记忆自我演化** — supersede 机制就绪但无人触发(P2 未实现);insert_fact 检测状态事实
  (kind='状态'+entity)→ 自动作废同主题旧状态。「用 SQLite」被「用 PostgreSQL」取代时旧的退场,
  brief 只见当前真相、记忆不堆矛盾;非状态(约定/教训)并存不消解。
- **经理可信度分级采信** — brief 早标可信度档,但经理 prompt 没点明用法 → 加引导(权威/机械=硬
  事实、员工汇报=参考、不可信=存疑,冲突信高档),可信度从标签变决策权重。
- **战绩接进经理决策层** — team 清单从「员工·专长」→「员工·专长·战绩 4/5(80%)」,prompt 引导
  难活给战绩好的、新人给机会。战绩在两层生效:pick_worker_role 机械派活 + 经理大脑拆活分配。
- **★ real 验证经理基于记忆改写任务** — 插约定「函数必须带 q_ 前缀」,派活**只说**「加回文函数」、
  没提前缀 → 真 claude 经理的 spawn 决策**主动把约定写进任务**(「要求:函数名必须带 q_ 前缀」)→
  worker 产出 `def q_is_palindrome(...)` → merged。经理读记忆→改写任务→worker遵循→交付,整条
  记忆驱动决策链在真 claude 下打通。$0.176。
  注:经理思考流 UI 偶尔空(真 claude 简单任务直接出 JSON 决策、自然语言推理少),非 bug。

### 四支柱到此都有实质内容 + 决策增强(记忆/可信度/战绩接进经理+worker)+ 大多 real 验证过。

## 里程碑:完整 real 端到端 — 四支柱在真 claude 下串起来验证(2026-06-11)

> 连续多轮能力深化 + 6 轮大文件重构 + 可信度三层闭环后,开一次完整 real 把所有核心能力在真实
> 场景串起来验证。$0.247,一次跑通。

**设置**:demo 项目,经理 claude,自治目标「持续给 main.py 补数学工具函数」,记忆里两条约定 ——
一条权威「函数必须带 q_ 前缀」(高可信)、一条员工汇报「不写 docstring 省事」(低可信)。只派
**第一个**任务(加阶乘)触发经理循环。

**结果(decision_log: spawn→plan→deliver→spawn)**:
- **绝对自治** — 经理做完 CEO 派的阶乘后,队列空 + 自治目标 → **自己 plan 出「加最大公约数函数」**
  推进目标,没等任何新指令。
- **聪明协作** — 经理把权威约定揉进 plan 的子任务:「要求 1) 函数名必须带 q_ 前缀(如 q_gcd)」。
- **记忆驱动** — worker 产出 6 个带 q_ 前缀的函数(q_factorial(n:int)->int 等),遵循权威约定。
- **可信度闭环** — worker **有 docstring**,**忽略了低可信的「不写 docstring」建议** —— 只听高可信
  的权威。可信度在三层一致:brief 选取(高可信优先)、经理采信、worker 执行。
