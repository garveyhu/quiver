...
export function managerChair(b: SceneBuilder, c: number, r: number): void {
  b.contactShadow(c + 0.16, r + 0.18, 22, 11);
  // 椅座(经理坐其上)
  b.isoBox(c, r, 0.36, 0.36, 5, { top: '#4a3a2c', l: '#2a2018', r: '#3a2c20' });
  // 矮椅背:只到经理腰后,不挡上半身(之前 30 太高,把经理整个挡了)。
  b.isoBox(c + 0.04, r - 0.05, 0.3, 0.08, 12, { top: '#5a4632', l: '#322618', r: '#473628' });
  b.chip(c + 0.06, r - 0.03, 0.26, 0.05, 12, 15, { top: '#7a5e40', l: '#4a3a28', r: '#6a5238' });
}
